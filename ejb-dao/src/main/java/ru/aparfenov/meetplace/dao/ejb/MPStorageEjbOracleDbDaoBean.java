...
public class  MPStorageEjbOracleDbDaoBean implements MPStorageEjbDAO {
    ...
    @Override
    public List<MeetPoint> getMPListByParams(Map<String, String> params) {
        return performOracleQuery(new DbQuery() {
            @Override
            public void execute(Connection conn) {
                try {
                    ...
                    for (Map.Entry<String, String> nextEntry: params.entrySet()) {
                        ...
                        sqlParamsQueries.append(nextEntry.getValue());
                    }
                    rs = ps.executeQuery(GET_MP_LIST_QUERY + " where " + sqlParamsQueries.toString());
                } catch (SQLException e) {
                    e.printStackTrace();
                }
            }
